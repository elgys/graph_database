...
pub mod table {
    ...
    impl TableHeader {
        pub fn check_correct(&self) -> Result<(), String> {
            if self.signature == 0x4442 && self.version == 0 {
                ...
                return Err("File has a incorrect header".into());
            }
        }

        pub fn write_to_file(&self, file: &mut fs::File) -> std::io::Result<()> {
            file.write_all(&self.serilize())?;
            Ok(())
        }
    }

    impl Serilize for TableHeader {
        fn serilize(&self) -> Vec<u8> {
            let mut file_binary: Vec<u8> = Vec::with_capacity(20);
            ...
            file_binary = vec![
                sig[0], sig[1], ver[0], ver[1], p_z[0], p_z[1], p_z[2], p_z[3], b_t_l[0], b_t_l[1],
                b_t_l[2], b_t_l[3], d_l[0], d_l[1], d_l[2], d_l[3], r_l[0], r_l[1], r_l[2], r_l[3],
            ];
            return file_binary;
        }
    }
    ...
    mod test_table {
        ...
        fn empty_file_header_text() {
            let test = correct_header();
            let text = test.serilize();
            let ans = vec![
                0x44u8, 0x42u8, 0x00u8, 0x00u8, 0x00u8, 0x00u8, 0x00u8, 0x00u8, 0x00u8, 0x00u8,
